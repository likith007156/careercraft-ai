...
const Progress = () => {
  ...
  if (loading || !data) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] text-text-secondary">
        ...
        <p className="text-xs font-mono">Drawing progress analytics graphs...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-12">
      ...
      {/* 1. Top row header */}
      <div className="bg-background-card border border-black/5 dark:border-white/5 p-6 rounded-card flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-card">
        <div className="flex items-center space-x-3">
          <BarChart3 className="text-primary" />
          <h1 className="text-lg font-serif font-bold text-text-primary">Performance Analytics Dashboard</h1>
        </div>
        
        <div className="bg-primary/10 py-1.5 px-4 rounded-badge border border-primary/20 text-xs font-bold text-primary font-mono text-center">
          Placement Readiness Indicator: {data.readiness_score}%
        </div>
      </div>

      {/* 2. Charts Grid (Sky Wash for cool charts) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        
        {/* Radar subject mastery (Cool blue accents) */}
            ...
            <Radar 
              data={{
                ...
                datasets: [{
                  ...
                  backgroundColor: 'rgba(59, 130, 246, 0.15)', // Cool blue fill
                  borderColor: '#3b82f6', // Cool blue border
                  borderWidth: 2,
        ...
        {/* Readiness line progression (Cool blue accents) */}
            ...
            <Line 
              data={{
                ...
                datasets: [{
                  ...
                  borderColor: '#3b82f6', // Cool blue border
                  backgroundColor: 'rgba(59, 130, 246, 0.05)', // Cool blue fill
                  tension: 0.3,
                  fill: true,
                  borderWidth: 2
                }]
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Mastery Map Grid (Cool data = Sky Wash / White, blue accents) */}
          ...
          <div className="grid grid-cols-6 sm:grid-cols-10 gap-2 overflow-y-auto max-h-[200px] pr-2">
            {data.mastery_map && data.mastery_map.map((topic, i) => (
              <div
                ...
                className={`aspect-square rounded-card flex items-center justify-center font-bold text-xs cursor-pointer border transition-all hover:scale-105 shadow-sm ${
        ...
        {/* AI Weekly Status Report (Warm data = Apricot Wash cards with Rust accents) */}
        <div className="bg-apricotWash border border-rust/10 dark:bg-background-card dark:border-white/5 p-6 rounded-card flex flex-col justify-between shadow-card text-rust dark:text-text-primary">
          <div>
            ...
            <p className="text-xs italic leading-relaxed font-serif font-medium">
              "{data.weekly_report}"
            </p>
          </div>
          ...
          <div className="mt-6 pt-3 border-t border-rust/15 dark:border-white/5 flex items-center justify-between text-xs font-bold font-sans">
            ...
            <span className="font-mono">{data.estimated_days_to_ready} Days</span>
          </div>
        </div>

      </div>

      {/* 5. Personal Flaw Detector (Subtle Rust accents) */}
      <div className="bg-white dark:bg-background-card border border-rust/20 dark:border-white/5 p-6 rounded-card space-y-4 shadow-card">
        <div className="flex justify-between items-center border-b border-black/5 dark:border-white/5 pb-2">
          <h3 className="font-serif font-bold text-sm text-text-primary flex items-center">
        ...
        {flaws && flaws.filter(f => !f.is_fixed).length > 0 ? (
          <div className="space-y-3">
            {flaws.filter(f => !f.is_fixed).map((flaw) => (
              <div key={flaw.id} className="p-4 rounded-input bg-danger-bg border border-danger/35 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
      ...
      {/* Selected subtopic detail overlay modal card */}
      {selectedSubtopic && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-background-card border border-black/5 dark:border-white/5 p-6 rounded-card max-w-sm w-full text-xs space-y-4 relative animate-scale-up shadow-card text-text-primary">
